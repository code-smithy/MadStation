...
Phase 5 is in progress with logistics + chain foundations now including 5B/5C controls: stricter `CreateWorkOrder` schema validation, deterministic item-chain stages (`RefineIce`/`FeedOxygenGenerator`), collision/replan behavior for competing item orders, and machine-coupled feed behavior (generator presence + powered gate). See `docs/phase5/PHASE5_SPLIT_PLAN.md`.
